...
void renderGame(State state) // Mo ta game
{
    cout << " -------------" << endl;
...
char getNextPlayer(State s) // xac dinh luot choi tiep theo
{
...
State play(State s, int i, int j, char player) // ham xac dinh nguoi choi hien tai
{
...
vector<State> getNextStates(State s) // ham xac dinh luot choi tiep theo
{
...
bool isFinalState(State s){ //xac dinh tro choi da ket thuc
    for (int i = 0; i < SIZE; i++)
    {
...
int getScoreFinalState(State s) // tinh diem buoc cuoi
{
    for (int i = 0; i < SIZE; i++)
        if (s[i][0] == s[i][1] && s[i][0] == s[i][2] && s[i][0] != ' ')
            return s[i][0] == 'X' ? 1 : -1;
...
pair<int, State> getScore(State s) //xac dinh nuoc di tot nhat
{
    if (isFinalState(s)) {
...
pair<int, int> getComputerPlay(State s)//may tinh choi
{
